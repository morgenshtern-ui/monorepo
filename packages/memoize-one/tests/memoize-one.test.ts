...
import { type EqualityFn, memoizeOne } from '../src/index.js'
...
describe('standard behaviour - baseline', () => {
  ...
  beforeEach(() => {
    add = vi.fn().mockImplementation((value1: number, value2: number): number => value1 + value2)
...
describe('standard behaviour - dynamic', () => {
  ...
  for (const { first, name, second } of inputs) {
    describe(`type: [${name}]`, () => {
      let mock: (...args: any[]) => unknown
      let memoized: (...args: any[]) => unknown

      beforeEach(() => {
        mock = vi.fn().mockImplementation((...args: any[]) => {
...
describe('respecting "this" context', () => {
  describe('original function', () => {
    it('should respect new bindings', () => {
      ...
      const memoized = memoizeOne((bar): any => {
        // @ts-expect-error: foo
        return new Foo(bar)
      })
  ...
  describe('memoized function', () => {
    ...
    it('should respect implicit bindings: 3', () => {
      const getAMemoized = memoizeOne(getA)
      const temp = {
        a: 2,
        getAMemoized,
...
describe('custom equality function', () => {
  ...
  it('should call the equality function with the newArgs, lastArgs and lastValue', () => {
    (equalityStub as any).mockReturnValue(true)
  ...
  it('should have a nice isDeepEqual consumption story', () => {
    ...
    const clone = (person: Person): Person => JSON.parse(JSON.stringify(person)) as Person
    const bob: Person = {
      age: 10,
    }
    const jane: Person = {
  ...
  it('should return execute and return the result of the result fn if the equality fn returns false', () => {
    (equalityStub as any).mockReturnValue(false)
...
describe('throwing', () => {
  ...
  it('should not memoize a thrown result', () => {
    ...
    expect(firstError).toEqual(new Error('hello'))
