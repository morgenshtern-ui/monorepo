...
class Swiper {
  constructor(...args) {
    ...
    if (
      ...
      for (const containerEl of document.querySelectorAll(params.el)) {
    ...
    for (const mod of swiper.modules) {
      mod({
        params,
        swiper,
    ...
    // add event listeners
    if (swiper.params?.on) {
      for (const eventName of Object.keys(swiper.params.on))
        swiper.on(eventName, swiper.params.on[eventName])
    }

    if (swiper.params?.onAny)
  ...
  getSlideIndexByData(index) {
    return this.getSlideIndex(
      this.slides.find(
        slideEl => Number(slideEl.dataset.swiperSlideIndex) === index,
      ),
    )
  }
  ...
  setProgress(progress, speed) {
    ...
    swiper.translateTo(current, speed === undefined ? 0 : speed)
  ...
  update() {
    ...
    for (const imageEl of swiper.el.querySelectorAll('[loading="lazy"]')) {
      if (imageEl.complete)
  ...
  mount(element) {
    ...
    if (el.parentNode?.host && el.parentNode.host.nodeName === 'SWIPER-CONTAINER')
    ...
    const getWrapper = () => {
      if (el?.shadowRoot?.querySelector) {
    ...
    if (!wrapperEl && swiper.params.createElements) {
      ...
      for (const slideEl of elementChildren(el, `.${swiper.params.slideClass}`))
  ...
  init(el) {
    ...
    for (const imageEl of lazyElements) {
      if (imageEl.complete) {
        processLazyPreloader(swiper, imageEl)
      }
      else {
  ...
  destroy(deleteInstance = true, cleanStyles = true) {
    ...
    if (swiper.params === undefined || swiper.destroyed)
      return null
    ...
    for (const eventName of Object.keys(swiper.eventsListeners))
  ...
  static installModule(mod) {
    ...
    if (typeof mod === 'function' && !modules.includes(mod))
      modules.push(mod)
  }

  static use(module) {
    if (Array.isArray(module)) {
      for (const m of module) Swiper.installModule(m)
...
for (const prototypeGroup of Object.keys(prototypes)) {
  for (const protoMethod of Object.keys(prototypes[prototypeGroup]))
