...
export default function onTouchMove(event) {
  ...
  if (swiper.params.threshold && Math.hypot(diffX, diffY) < swiper.params.threshold)
    return
  ...
  if (diff > 0) {
    ...
    if (
      ...
      && data.currentTranslate
      < (params.centeredSlides
        ? swiper.maxTranslate() + swiper.slidesSizesGrid.at(-1)
        : swiper.maxTranslate())
    ) {
      swiper.loopFix({
        direction: 'next',
        ...
        activeSlideIndex:
          ...
          - (params.slidesPerView === 'auto'
            ...
            : Math.ceil(Number.parseFloat(params.slidesPerView, 10))),
      })
    }
  ...
  if (
    (params.freeMode?.enabled && swiper.freeMode)
  ...
  if (params.freeMode?.enabled && swiper.freeMode)
