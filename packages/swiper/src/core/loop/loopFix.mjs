...
export default function loopFix({
  ...
  if (slidesPerView === 'auto') {
    ...
    slidesPerView = Math.ceil(Number.parseFloat(params.slidesPerView, 10))
  ...
  const activeColIndexWithShift
    = activeColIndex
    + (centeredSlides && setTranslate === undefined ? -slidesPerView / 2 + 0.5 : 0)
  ...
  if (isPrev) {
    for (const index of prependSlidesIndexes) {
  ...
  if (isNext) {
    for (const index of appendSlidesIndexes) {
      slides[index].swiperLoopMoveDOM = true
  ...
  if (params.slidesPerView === 'auto') {
    ...
    for (const [slideIndex, slide] of swiper.slides.entries())
  ...
  if (swiper.controller?.control && !byController) {
    const loopParams = {
      slideRealIndex,
      direction,
      setTranslate,
    ...
    if (Array.isArray(swiper.controller.control)) {
      for (const c of swiper.controller.control) {
